          ...
          $.ajax({
              ...
              debugger;
               console.log(response);
                  ...
                  // let book = new Book(response.id, response.title, response.author,	response.numPages,response.pubDate,	response.cover, "X");
                  let book = new Book(response.id, response.title, response.author, response.numPages, response.pubDate, response.cover);

                  // self.myBookArray.push(book);
                        // self._addBook(
                        // 	new Book(
                        // 		response[i].title,
                        // 	)BOOK
                    ...
                    debugger;
                     console.log(response);
                      for (var i = 0; i < response.length; i++) {
                        // let book = new Book(response[i].id, response[i].title, response[i].author,	response[i].numPages,response[i].pubDate,	response[i].cover, "X");
                           let book = new Book(response[i].title, response[i].author,	response[i].numPages,response[i].pubDate,	response[i].cover, "X");
                        self.myBookArray.push(book);
                        			// self._addBook(
                        			// 	new Book(
                        			// 		response[i].title,
...
class Book {
       constructor(id, title, author, numPages, pubDate, cover) {
